...
Script Name: RPS_Position.cs

Description:
This script can be attached to any object, to calculate the object’s current race position.
This script does the majority of the work to calculate the object's position based on PositionSensors around the track.
It is required by all Lap and UI scripts.
*/
...
public class RPS_Position : MonoBehaviour {
	...
	public float currentNearestPosition; //Position number of the nearest PositionSensor
	public float currentNearestPositionUnlimited; //Position number of neareast PositionSensor whithout limits from RPS_Checkpoints script
	public float percentageDist; //Percentage of the way between the last and next PositionSensors. This is used when two RPS_Position scripts are between the same two Position Sensors for higher accuracy.
	...
	public int currentRacePosition = 0; //The current calculated race position. 0 = 1st, 1 = 2nd, ... etc
	public bool useLaps = false; //Whether there is a RPS_Lap script attached to the object to use laps or not to calculate position
	public int currentLapNumber = 0; //What lap it is currently on if useLaps=true
	public int lapsGoneBack = 0; //Used if a RPS_Position object does a lap backwards and passes the start/finish in the wrong direction. It doesn't change the current lap number, but it does mean the player has to do another lap in the right direction to get back to where they were.
	...
	private bool useCheckpoints = false; //Whether there is a RPS_Checkpoints script attached to this object. This is checked at runtime.
	private RPS_Checkpoints thisCheckpoints; //The instance of RPS_Checkpoints if attached to this object.

	private float lastFrameNearestPosition; //The nearest position number in the last frame. If it was the very last position number of a lap and it is the first position number in this frame, it shows a lap has been completed or end of race reached.
    private bool isFirstFrame = true; //Prevents the script trying to access the lastFrameNearestPosition variable when one doesn't exist

	public bool hasFinished = false; //This is used to compare object positions. When one object has finished the race and the other hasn't, then the one which has finished is ahead.
	private bool hasFinishedLastFrame = false;

	private bool freezeRacePos = false;
